//
//  CityData.swift
//  TTDRecommendationsExcercise
//
//  Created by Chase Holland on 5/10/22.
//
...
cityList.forEach(item => {
    // Give the cities a unique ID and label.
    item.id = item.city + item.rank;
    item.label = `${item.city}, ${item.state}`;
